...
# *                                                                     *
# * Purpose: Evaluate the trained model on original traces              *
#  by computing accuracy and mean rank                                  *
# ***********************************************************************
...
def compute_score(test_traces, plaintext):
    ...
    It receives the traces of the attack phase then computes the score as follow:
        ...
        3- For each trace we sum up the logarithm of prediction[j][class number].
...
def plot_signal(x_signal, y_signal, class_number):
    ...
    plt.title("Class {}".format(y_signal.argmax(1)[class_number]))# print the labels
    plt.show()
...
# **************************************
compTracefile = h5py.File('../Compressed_Traces/Compressed_Traces.hdf5', 'r')  # read
X_train_original = np.array(compTracefile.get('C_T'))
...
# *****************************************
model = load_model("Model_Original_Traces_256.hdf5")
model.load_weights("Best_weights_Original_Traces_256.hdf5")
